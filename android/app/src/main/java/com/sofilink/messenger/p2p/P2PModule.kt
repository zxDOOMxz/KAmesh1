...
class P2PModule(reactContext: ReactApplicationContext) :
  ...
  fun getPeerId(promise: Promise) {
    peerId?.let { promise.resolve(it) } ?: promise.reject("NOT_INIT", "P2P not initialized")
  }
  ...
  fun startServer(port: Int, promise: Promise) {
    scope.launch {
      try {
        ...
        val acceptJob = launch {
          while (isActive && !server.isClosed) {
            try {
              ...
              if (!server.isClosed) Log.e("SofiLink/P2P", "Accept error", e)
            }
          }
        }
        activeJobs["accept_$serverId"] = acceptJob

        val localIp = NetworkInterface.getNetworkInterfaces()
          ?.asSequence()
        ...
        result.putString("serverId", serverId)
        result.putString("localIp", localIp)
  ...
  fun connect(host: String, port: Int, promise: Promise) {
    scope.launch {
      try {
        ...
        Log.e("SofiLink/P2P", "Connect failed to $host:$port", e)
  ...
  fun sendMessage(connectionId: String, data: String, promise: Promise) {
    try {
      val peer = peers[connectionId]
        ?: return promise.reject("PEER_NOT_FOUND", "No peer with id $connectionId")

      val msg = buildDataMessage(data)
      peer.socket.getOutputStream().write(msg)
  ...
  private fun handleConnection(socket: Socket) {
    scope.launch {
      try {
        ...
        val connId = "incoming_${socket.inetAddress.hostAddress}_${socket.port}"
        ...
        Log.e("SofiLink/P2P", "Connection handler error", e)
      } finally {
  ...
  private fun buildHandshakeMessage(): ByteArray {
    val pk = identityKeyPair?.public?.encoded ?: byteArrayOf()
    return "HANDSHAKE:${bytesToHex(pk)}\n".toByteArray(Charsets.UTF_8)
  }

  private fun buildDataMessage(data: String): ByteArray {
    return "MSG:$data\n".toByteArray(Charsets.UTF_8)
  }
